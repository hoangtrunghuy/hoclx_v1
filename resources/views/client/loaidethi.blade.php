<section class="bgwhite p-t-70 p-b-40">
    <div class="container">
        <div class="row">
            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/b1.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05 ">
                        <a href="#" class="wrap-title-b1">
                            <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                            <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng B1</h4>
                        </a>

                        <div class="wrap-para-b1 p-l-50 p-r-20 m-tt-20 ">
                            <p class="s-txt1">
                                Thi sát hạch hạng B1: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 26 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-50 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/b2.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05">
                        <a href="#">
                            <div class="wrap-title-b1">
                                <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                                <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng B2</h4>
                            </div>
                        </a>

                        <div class="wrap-para-b1 p-l-37 p-r-20 m-tt-20">
                            <p class="s-txt1">
                                Thi sát hạch hạng B2: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 26 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-37 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/c.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05">
                        <a href="#">
                            <div class="wrap-title-b1">
                                <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                                <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng C</h4>
                            </div>
                        </a>

                        <div class="wrap-para-b1 p-l-50 p-r-20 m-tt-20">
                            <p class="s-txt1">
                                Thi sát hạch hạng C: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 28 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-50 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <div class="container">
        <div class="row">
            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/d.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05 ">
                        <a href="#" class="wrap-title-b1">
                            <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                            <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng D</h4>
                        </a>

                        <div class="wrap-para-b1 p-l-50 p-r-20 m-tt-20 ">
                            <p class="s-txt1">
                                Thi sát hạch hạng D: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 28 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-50 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/e.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05">
                        <a href="#">
                            <div class="wrap-title-b1">
                                <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                                <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng E</h4>
                            </div>
                        </a>

                        <div class="wrap-para-b1 p-l-37 p-r-20 m-tt-20">
                            <p class="s-txt1">
                                Thi sát hạch hạng E: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 28 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-37 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col-sm-9 col-md-7 col-lg-4 p-b-30 m-lr-auto">
                <!-- Block 1 -->
                <div class="block-1">
                    <div class="wrap-pic-b1 wrap-pic-w">
                        <img src="client/images/f.jpg" alt="IMG-FEATURE">
                    </div>

                    <div class="wrap-text-b1 bg-main op-09 trans-05">
                        <a href="#">
                            <div class="wrap-title-b1">
                                <i class="fa fa-id-card color-white fs-30" aria-hidden="true"></i>
                                <h4 class="m-txt3 color-white tt-up m-l-15">Sát hạch hạng F</h4>
                            </div>
                        </a>

                        <div class="wrap-para-b1 p-l-50 p-r-20 m-tt-20">
                            <p class="s-txt1">
                                Thi sát hạch hạng F: Tổng số 30 câu hỏi, trả lời trong 20 phút.
                                Thi đạt nếu số câu trả lời đúng >= 28 / 30 câu.
                            </p>
                        </div>

                        <div class="wrap-more-b1 p-l-50 p-t-10">
                            <a href="#" class="s-txt1 hov-color-white m-tt-10">
                                <i class="fa fa-caret-right fs-16 trans-04" aria-hidden="true"></i>
                                <span class=" trans-04">BẮT ĐẦU</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>